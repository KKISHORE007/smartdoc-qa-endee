...
function saveProfilePhotoToFile(base64Image, userIdentifier) {
    ...
    const filename = `profile_${userIdentifier}_${Date.now()}.${extension}`;
...
router.post('/register', async (req, res) => {
    try {
        const { firstName, lastName, age, contactMethod, password } = req.body;
        ...
        // Convert base64 to file URL if necessary
        const safeContactMethod = contactMethod.replace(/[^a-zA-Z0-9]/g, '');
        profilePhoto = saveProfilePhotoToFile(profilePhoto, safeContactMethod);
...
router.put('/profile', async (req, res) => {
    try {
        ...
        if (profilePhoto !== undefined) {
            user.profilePhoto = saveProfilePhotoToFile(profilePhoto, user.id);
        }
