...
function App() {
  ...
  const handleChangeDocument = useCallback(() => {
    const saveEntry = (msgs, setHistory) => {
    ...
    saveEntry(offlineMessages, setOfflineChatHistory);
    saveEntry(onlineMessages, setOnlineChatHistory);
  ...
  }, [uploadedFile, offlineMessages, onlineMessages, readPreview, localDocText]);
  ...
  const handleRestoreHistory = useCallback(async (histEntry) => {
    ...
    // Remove the restored entry from history to avoid duplicates
    setChatHistory(prev => prev.filter(e => e !== histEntry));
  }, [uploadedFile, messages, readPreview, chatMode, localDocText]);
              ...
              {forgotStep !== 1 && (
                <div className="space-y-1.5">
                  <label className="text-xs font-bold tracking-wide" style={{ color: 'var(--text-secondary)' }}>
                    {forgotStep === 2 ? 'NEW STRONG PASSWORD' : 'STRONG PASSWORD'}
                  </label>
                  <input type="password" required placeholder="••••••••" minLength="8"
                         ...
                         onFocus={e => e.target.style.borderColor = 'var(--accent-primary)'}
                         onBlur={e => e.target.style.borderColor = 'var(--border-primary)'} />
                  {(!isLoginMode || forgotStep === 2) && <p className="text-[10px]" style={{ color: 'var(--text-muted)' }}>Must be at least 8 characters long.</p>}
                </div>
              )}
              ...
              <div className="pt-4 space-y-3">
                ...
                {forgotStep === 0 ? (
                  <div className="text-center">
                    ...
                    <button 
                      type="button" 
                      onClick={() => { setIsLoginMode(!isLoginMode); setAuthForm({ firstName: '', lastName: '', age: '', contactMethod: '', password: '', profilePhoto: '' }); }}
